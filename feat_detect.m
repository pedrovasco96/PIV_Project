function [feat_d,foreg]=feat_detect(BG1,BG2,imseq1,imseq2,x,y,n,cam_params,subfolder)
feat_d=struct('frame_cam_1',cell(n,1),'desc_cam_1',cell(n,1),'frame_cam_2',cell(n,1),'desc_cam_2',cell(n,1));
foreg=struct('foreground_cam_1',cell(n,1),'foreground_cam_2',cell(n,1),'obj_cam_1',cell(n,1),'obj_cam_2',cell(n,1));
figure();
for i=1:n
    ...
    labels1=squeeze(bwlabel(fg1,8));
%     n1=max(labels1);
    labels2=squeeze(bwlabel(fg2,8));
    foreg(i).foreground_cam_1=cat(2,xyz1(fg1,:),labels1(fg1));
    foreg(i).foreground_cam_2=cat(2,xyz2(fg2,:),labels2(fg2));
    img1=rgb2gray(rgbd1).*uint8(reshape(fg1,[x,y]));
    img2=rgb2gray(rgbd2).*uint8(reshape(fg2,[x,y]));
    imshow(img1);
